...
readRDS(file = "data/regio.rds") %>% 
  ...
  labs(x = "Wahlbeteiligung (in Prozent)",
       ...
       title = "Wahlerfolg der AfD in Abhängigkeit der Wahlbeteiligung",
...
Mit R lassen sich auf ganz unterschiedliche Weise Grafiken erstellen. Die Art und Weisen unterscheiden sich dabei hinsichtlich Komplexität und aber auch Möglichkeiten. Diese Varianten gibt es:
...
plot(x = df_regio$Europawahl_Beteiligung_percent, 
     y = df_regio$Europawahl_AFD_percent,
     main = "Wahlstimmer für die AfD in Abhängigkeit der Wahlbeteiligung",
...
```
Die Idee dabei ist es, den Plot als ein Schichtung verschiedener Ebenen zu betrachten. Wenn man mit `ggplot` ein Grafik erstellen will, ist es hilfreich, sich dieses Bild vor Augen zu führen. Die unterste Ebene sind die Daten. Dann entscheiden wir uns mit den Aesthetics, welche Variablen auf welche Achsen abgebildet werden sollen (mapping). Die Geome bestimmen, wie der zuvor definierte Raum dargestellt werden soll. Sie geben den Daten eine "Form", wie zum Beispiel im einfachsten Fall Punkte. Nach den Geomen folgen die Facets, welche die Möglichkeit bieten die vorher definierte Darstellung anhand einer bestimmten "Facette" darzustellen. Beispielsweise könnten wir hier, nach einem bestimmten kategorialen Merkmal stratifizieren, sodass jede Kategorie einen neuen Sub-Plot darstellt. Zuletzt können wir dann noch das gesamte Erscheinungsbild des Plots anpassen, mit den "Coordinates" und "Themes". Soviel zur Theorie! Wir werden sehen, dass wir nicht alle Ebenen immer brauchen. 
...
Diese Vorgehensweise macht euren Code noch übersichtlicher und ihr "ramscht" 
eure Global Environment nicht voll.
...
Für weitere Geome siehe [Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/data-visualization-2.1.pdf).
...
Mithilfe von **Facets** lassen sich mehrere Plots in einer Graphik darstellen. Hierfür nutzen wir das Geom `facet_grid()`. Die Daten werden hierbei nach den 
übergebenen Variablen gruppiert und es wird jeweils ein Plot für jedes Merkmal zur Grafik hinzugefügt. Dabei können die Facets in Spalten (`cols`) oder Zeilen (`rows`) angeordnet werden.
...
Erstellt einen Boxplot für den Prozentsatz der Wahlstimmen der AfD. Stellt jeweils ein Plot für die alten und für die neuen Bundesländer nebeneinander dar.

Speichert diese Grafik im Anschluss in dem Ordner `plots` unter dem Namen 
`vote_afd.pdf`.
...
```{r, eval=FALSE}
plot_afd <- df_regio %>% 
  ggplot(mapping = aes(y = Europawahl_AFD_percent)) +
  geom_boxplot() +
  facet_grid(cols = vars(Region))

ggsave(plot = plot_afd,
       filename = "plots/vote_afd.pdf")
```
...
**Problem:** Werte für Burundi "kleben" am Boden der Grafik und sind so 
...
Für viele nicht sichtbar: die Standardfarben von GGPlot sind nicht geeignet für Personen mit Farbenblidheit. Hier hilft das Package `viridis`.
...
Reproduziert den letzten Plot für das Jahr **1962** und zoomt mit `facet_zoom()` an die Europäischen Staaten ran.

Probiert dabei aus, was passiert, wenn ihr statt `xy` als Argument nur `x` oder nur `y` verwendet. Was bietet sich am besten an?
...
Bei Inspirationslosigkeit liefert die [\textcolor{blue}{R Graph Gallery}](https://r-graph-gallery.com) Vorschläge für unterschiedliche 
Typen von Grafiken.
...
Von dieser Sitzung solltest du olgendes mitgenommen haben:
